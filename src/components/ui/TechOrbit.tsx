...
// SiOpenai absent from this package version — same inline SVG used in Skills.tsx
const OpenAIIcon = () => (
...
const ICONS = [
  { Icon: SiReact,     label: "React",    primary: true  },
  { Icon: SiAngular,   label: "Angular",  primary: false },
  { Icon: SiLaravel,   label: "Laravel",  primary: true  },
  { Icon: SiPython,    label: "Python",   primary: false },
  { Icon: SiDocker,    label: "Docker",   primary: true  },
  { Icon: SiNextdotjs, label: "Next.js",  primary: false },
  { Icon: SiSupabase,  label: "Supabase", primary: true  },
  { Icon: OpenAIIcon,  label: "OpenAI",   primary: false },
] as const;

// Orbit radius expressed as a CSS min() so the ring scales down on narrow viewports.
// On desktop (≥440px container): min(200px, 45vw) = 200px.
// On 375px mobile: 45vw ≈ 169px — chips stay within the responsive container.
const RADIUS = "min(200px, 45vw)";
const CHIP = 36;
const HALF = CHIP / 2; // 18
...
export default function TechOrbit({ className = "" }: TechOrbitProps) {
  return (
    <div className={`pointer-events-none ${className}`} aria-hidden="true">
      {/* Subtle orbit-track guide circle */}
      <div className="absolute inset-0 rounded-full border border-white/5 dark:border-white/5" />

      {/* Ring — this div rotates, carrying all icon anchors with it */}
      <div
        className="absolute inset-0"
        style={{ animation: "orbit-spin 20s linear infinite" }}
      >
        {ICONS.map(({ Icon, label, primary }, i) => {
          ...
          return (
            ...
            >
              {/* Chip — centered on the anchor point, counter-spins to stay upright */}
              <div
                className={[
                  "flex items-center justify-center rounded-full",
                  "bg-dark-900/85 dark:bg-dark-800/90",
                  "border border-dark-700/40 shadow-md",
                  primary ? "text-primary-500" : "text-violet-500",
                ].join(" ")}
                style={{
                  width: CHIP,
                  height: CHIP,
